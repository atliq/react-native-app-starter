...
const PackageManagerService_1 = __importDefault(require("./PackageManagerService"));
class DependencyService {
     ...
     */
    static async processDependenciesWithVersionCheck(directoryName, tempDependencies, tempDevDependencies) {
        const projectPackageJsonPath = `${directoryName}/package.json`;
        const projectPackageJson = (0, fileSystem_1.readJsonFile)(projectPackageJsonPath);
        ...
        for (const [depName, tempVersion] of Object.entries(tempDependencies)) {
            const packageToInstall = this._shouldInstallDependency(depName, tempVersion, projectDependencies[depName], "dependency");
            if (packageToInstall) {
        ...
        for (const [depName, tempVersion] of Object.entries(tempDevDependencies)) {
            const packageToInstall = this._shouldInstallDependency(depName, tempVersion, projectDevDependencies[depName], "dev dependency");
            if (packageToInstall) {
                devDependenciesToInstall.push(packageToInstall);
            }
        }
        console.log(`\nDependency processing complete:`);
        console.log(`- ${dependenciesToInstall.length} regular dependencies to install/update`);
        console.log(`- ${devDependenciesToInstall.length} dev dependencies to install/update`);
     ...
     * @private
     */
    static _shouldInstallDependency(depName, tempVersion, existingVersion, depType) {
        if (!existingVersion) {
            // Dependency doesn't exist in project, add it
            console.log(`Adding new ${depType}: ${depName}@${tempVersion}`);
            return `${depName}@${tempVersion}`;
        }
        if ((0, version_1.isStaticVersion)(tempVersion)) {
            // Temp has static version, check if project version is higher
            const comparison = (0, version_1.compareVersions)(existingVersion, tempVersion);
            if (comparison < 0) {
                // Project version is lower, use temp version
                console.log(`Updating ${depType} ${depName} from ${existingVersion} to ${tempVersion} (static version from template)`);
                return `${depName}@${tempVersion}`;
            }
            else {
                console.log(`Keeping existing ${depType} ${depName}@${existingVersion} (version ${comparison === 0 ? "same as" : "higher than"} template's ${tempVersion})`);
                return null;
            }
        }
        else {
            ...
            const comparison = (0, version_1.compareVersions)(existingVersion, tempVersion);
            if (comparison < 0) {
                console.log(`Updating ${depType} ${depName} from ${existingVersion} to ${tempVersion} (template version)`);
                return `${depName}@${tempVersion}`;
            }
            else {
                console.log(`Keeping existing ${depType} ${depName}@${existingVersion} (version ${comparison === 0 ? "same as" : "higher than"} template's ${tempVersion})`);
     ...
     */
    static installDependencies(directoryName, dependencies, devDependencies, shell, packageManager = "bun") {
        const shellOptions = {
            cwd: `${process.cwd()}/${directoryName}`,
        };
        if (dependencies.length > 0) {
            console.log(`Installing dependencies with ${packageManager}:`, dependencies);
            const addCommand = PackageManagerService_1.default.getCommand(packageManager, "add");
            console.log(`Running: ${addCommand} ${dependencies.join(" ")}`);
            shell.exec(`${addCommand} ${dependencies.join(" ")}`, shellOptions);
        }
        if (devDependencies.length > 0) {
            console.log(`Installing dev dependencies with ${packageManager}:`, devDependencies);
            const addDevCommand = PackageManagerService_1.default.getCommand(packageManager, "add-dev");
     ...
     * @param packageManager - Package manager to use
     */
    static fallbackInstallation(directoryName, tempDependencies, tempDevDependencies, shell, packageManager = "bun") {
        ...
        if (devDependencyList.length > 0) {
            const addDevCommand = PackageManagerService_1.default.getCommand(packageManager, "add-dev");
            shell.exec(`${addDevCommand} ${devDependencyList.join(" ")}`, shellOptions);
        }
    }
}
